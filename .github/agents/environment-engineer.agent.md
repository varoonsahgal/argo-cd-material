...
description: Design, write, and locally validate the per-student lab infrastructure — Terraform-provisioned VMs, the two-cluster (management + workload) Kubernetes topology, Argo CD installation, sample repositories, and reset/fault-injection tooling — plus the instructor and student setup guides.
argument-hint: "Provide the blueprint's environment specification and the target student/instructor VM counts."
