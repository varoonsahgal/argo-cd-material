...
| 3 | `lab-01-follow-an-application-through-reconciliation` | 45 | Environment check and login 7, guided walkthrough 10, E1 5, E2 10, E3 7, E4 4, checkpoint/debrief 2 | **TIGHT.** Relies on a pre-created Application, the 60 s reconciliation interval, and a credentials file on the VM. |
...
- *Exercises:*
  ...
  - E2 (10 min): **predict, then commit**. Change `message` in `chart/values.yaml`, then predict the sync/health status at four moments (after push, after Refresh, after Sync, after the rollout). Success: the Synced revision equals `git rev-parse HEAD`, and `curl` returns the new message.
...
- *Screenshots:* SS-L1-01 to SS-L1-10.
