...
> - your SSH (Secure Shell) session to your VM (virtual machine), from the student setup guide;
> - a second SSH session to the same VM (useful for port-forwards and for watching status while you work);
> - a browser with the Argo CD tunnel running (`https://localhost:8443`), logged in as `admin`;
> - a text editor for your incident log (`nano` on the VM works, and so does an editor on your laptop).
>
...
Open it with `nano ~/capstone/incident-log.md` (save with `Ctrl+O`, then `Enter`; exit with `Ctrl+X`), or copy it to your laptop and edit it there.
...
| `capstone-check.sh: command not found`, or it reports a missing tool | The course tooling is not available in this shell. | Tell your instructor. Do not go looking for the script's files. |
